...
# File location
input_file = "KitsuneInput.csv"
converted_file = "KitsuneInput.tsv"
...
output_file = "output_with_rmse.tsv"
...
with open(path, 'r') as f_input, open(output_file, 'w') as f_output:
    ...
    while True:
        ...
        flow_id = columns.pop(-3)
        label = columns.pop(-2)
        source_file = columns.pop(-1)

        # Process the modified line with Kitsune
        rmse = K.proc_next_packet()

        # Append the RMSE value and the three columns back
        columns.extend([flow_id, label, source_file, str(rmse)])
...
benignSample = np.log(RMSEs[FMgrace+ADgrace+1:49861])
