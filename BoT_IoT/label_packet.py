...
def collapse_flows(filename):
    ...
    with open(filename, 'r') as f:
        ...
        for row in reader:
            ...
            key = tuple(row[headers.index(i)] for i in ["saddr", "sport", "daddr", "dport", "proto", "stime", "ltime"])
            ...
            if existing:
                ...
                if row[headers.index("attack")] != '0':
                    existing["label"] = 'ATTACK'
                    existing["category"] = row[headers.index("category")]
            else:
                # Add new record
                collapsed[key].append({
                    "start_time": row[headers.index("stime")],
                    "end_time": row[headers.index("ltime")],
                    "label": 'ATTACK' if row[headers.index("attack")] != '0' else 'BENIGN',
    ...
    with open(new_filename, 'w', newline='') as f:
        ...
        for key, entries in collapsed.items():
            for entry in entries:
                new_row = list(key[:-2]) + [entry["start_time"], entry["end_time"], entry["label"], entry["category"]]
