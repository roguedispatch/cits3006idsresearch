...
class SplitData(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, labels, one_hot_label=True):
        if self.dataset == 'nsl':
            ...
            # KDDCUP99 dataset processing
            y_ = X[labels]
            X_ = X.drop(['label', 'label_string'], axis=1)
            y_ = np.where(y_ == 0, 0, 1).astype('float32')
        elif self.dataset == 'mirai':
            y = X[labels]
            X_ = X.drop(['label'], axis=1)
            y_ = np.asarray(y).astype('float32')
            ...
            y_ = X[labels]
            X_ = X.drop(['label'], axis=1)
            y_ = np.where(y_ == "BENIGN", 0, 1).astype("float32")            
        elif self.dataset == 'botiot':
            y_ = X[labels]
            X_ = X.drop(['label'], axis=1)
            X_ = X_.drop(['saddr', 'sport', 'daddr', 'dport'], axis=1, errors='ignore')
            y_ = np.where(y_ == "Normal", 0, 1).astype("float32")
        elif self.dataset == 'ctu13':
            y_ = X[labels]
            y_ = np.asarray(y_).astype('float32')
            X_ = X.drop(['label'], axis=1)
            X_ = X_.drop(['SrcAddr', 'Sport', 'DstAddr', 'Dport'], axis=1, errors='ignore')
            # X_ = pd.get_dummies(X_, columns=['State', 'Proto'], drop_first=False)
        else:
...
def score_detail(y_test,y_test_pred,if_print=False):
    ...
    return accuracy_score(y_test, y_test_pred), precision_score(y_test, y_test_pred), recall_score(y_test, y_test_pred), f1_score(y_test,y_test_pred), confusion_matrix(y_test, y_test_pred)
